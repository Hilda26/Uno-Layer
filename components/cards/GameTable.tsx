...
export default function GameTable({ gameState, myAddress, onDraw, isSubmitting }: Props) {
  const { players, currentTurnWallet, activeColour, activeDiscard, drawPileRemaining, deckSeed } = gameState;
    ...
    >
      ...
      {deckSeed && (
        <div className="absolute top-3 right-3 flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold"
          ...
          title={`Deck seed: ${deckSeed}`}
        >
          <span>🔗</span>
          <span>Deck generated by GenLayer consensus</span>
        </div>
      )}
